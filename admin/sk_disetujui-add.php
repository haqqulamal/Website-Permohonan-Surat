          ...
          <div class="row">
              ...
              <div class='box box-info'>
                <div class='box-header with-border'>
                  <h3 class='box-title'>Tambah Data SK Disetujui</h3>
                </div>
              <div class='box-body'>
              <form method='POST' class='form-horizontal' action='sk_disetujui-aksi' enctype="multipart/form-data">
                <div class='col-md-12'>
                  <table class='table table-condensed table-bordered'>
                  <tbody> 

                <tr><th scope='row'>Pilih No Surat Yang Ada Persetujuan</th> <td>
                <select class='form-control' name='id_surat' required oninvalid="this.setCustomValidity('Data tidak boleh kosong')" 
                oninput="setCustomValidity('')">
                <option hidden>Pilih No Surat</option>
                <?php
                include "../koneksi.php";

                //display values in combobox/dropdown
                $result = mysqli_query($konek,"SELECT * FROM persetujuan_permohonan WHERE status_persetujuan = 'disetujui'");
                  while($row = mysqli_fetch_assoc($result)){
                     echo "<option value=$row[id_surat]>$row[id_surat] - $row[catatan]</option>";
                    }
                ?>
                </select>
                </td></tr>  
                 ...
                 <tr><th scope='row'>nomor_sk</th> <td><input type='text' class='form-control' name='nomor_sk' 
                      required oninvalid="this.setCustomValidity('Data tidak boleh kosong')" oninput="setCustomValidity('')"> </td></tr>   
